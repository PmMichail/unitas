...
## 31. Реконструкция Маркетплейса, Выбор Бухгалтера при Заказе и Вкладка «Входящие запросы»

### 1. Рефакторинг карточек Маркетплейса (`/marketplace`)
- **Удаление тарифов с превью**: Полностью убрали списки тарифов, цен и кнопки «Заказать сопровождение» с главных карточек компаний. Теперь там отображается только основная информация: Логотип, Название компании, Рейтинг, Биография (public_bio) и контакты (email, телефон).
- **Модальное окно выбора тарифов**: При клике на карточку компании открывается всплывающее окно (modal overlay) со списком всех её тарифов, цен, описаний и кнопкой «Заказать сопровождение» для каждого тарифа.

### 2. Выбор исполнителя (бухгалтера) при оформлении заказа
- **Выпадающий список**: В модальное окно оформления заказа (Checkout modal) добавлен выпадающий список активных бухгалтеров компании.
- **Опция по умолчанию**: Реализована галочка «На усмотрение компании (рекомендовано)» — если она выбрана, исполнитель назначается автоматически из числа свободных сотрудников.

### 3. Панель «Входящие запросы от клиентов» в кабинете компании
- **Вкладка в дашборде**: Добавлена новая вкладка «Вхідні запити» в панели владельца консалтинга.
- **Индикатор новых заявок**: На вкладке выводится анимированный значок (badge) с количеством запросов со статусом `requested`.
- **Назначение и подтверждение**: Владелец видит детали заявки, может выбрать конкретного активного бухгалтера из своей команды и подтвердить сотрудничество кнопкой «Подтвердить», после чего клиенту выставляется счет.

### 4. Исправление ошибки «Ошибка загрузки договора»
- **Устранение race condition**: Запрос создания соглашения в API [main.py](file:///Users/mac/.gemini/antigravity-ide/scratch/unitas/backend/api/main.py) обернут в блок `try-except` для безопасной обработки конкурентных запросов вставки, предотвращая ошибки дублирования ключей (`IntegrityError`). В случае сбоя транзакция откатывается и возвращается уже созданная запись.
