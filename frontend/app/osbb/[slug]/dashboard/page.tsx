              ...
              <div className="grid gap-4 md:grid-cols-2">
                {surveys.map((survey) => (
                  <div key={survey.id} className="rounded-2xl border border-slate-200 p-4 bg-slate-50/50 flex flex-col justify-between">
                    ...
                    <div className="mt-5 pt-3 border-t border-slate-200/60 flex flex-wrap gap-2">
                      {["for", "against", "abstain"].map((value) => (
                        <button key={value} onClick={() => vote(survey.id, value)} className={`rounded-xl px-4 py-2.5 text-xs font-bold transition-all active:scale-95 ${survey.own_vote === value ? "bg-indigo-600 text-white shadow-md" : "bg-white border border-slate-200 text-slate-650 hover:bg-slate-50"}`}>
                          {survey.own_vote === value && <Check className="mr-1 inline w-3.5 h-3.5" />}{voteLabels[value]}
                        </button>
                      ))}
                    </div>
                  </div>
